...
fn dns_op_is_exhaustive_and_round_trips_all_16() {
    for v in 0u128..16 {
        let op = <Op as bits::Bits>::from_bits(v);
        assert_eq!(<Op as bits::Bits>::into_bits(op), v, "op {v}");
    }
}
...
fn bitfield_seam_exposes_layout_metadata() {
    use bits::{BitOrder, Bitfield, ByteOrder};
...
fn nested_bitfield_implements_bits_for_composition() {
    ...
    assert_eq!(<OpCode as bits::Bits>::BITS, 5);
    assert_eq!(<Flags as bits::Bits>::BITS, 7);
    ...
    assert_eq!(bits::Bits::into_bits(op), 0b1_0101);
}
