...
1. **Scope** — read the `design_preview` + this entry; identify what it touches
   (runtime `bnb/src/bitstream.rs` / macro `bnb-macros/src/bitstream.rs` / tests).
...
5. **Gate** (all clean): `RUSTC_WRAPPER= cargo test -p bnb`, `… --no-default-features`,
   `… clippy -p bnb-macros -p bnb --all-targets` (`clippy::all = deny`),
   `… fmt --all --check`.
...
- [x] `binrw`/`binrw_derive` are out of the default dependency graph (`cargo tree
      -p bnb -e normal` shows no binrw). `deny.toml` stays valid (binrw is still used
      by protocol crates + the opt-in feature); `Cargo.lock` tracked; `ACKNOWLEDGMENTS.md`
      kept.

**Exit ✓ (achieved):** `cargo tree -p bnb` shows no default `binrw`; `bits` tests
pass on default (44 groups), `--features binrw`, and `--features bytes`.

## Phase 5 — Rename `bits` → `bnb`

Mechanical, now that the codec is genuinely owned (DD5):

- [x] Renamed crates `bits`→`bnb`, `bits-macros`→`bnb-macros` (`git mv`); `bnb::bin`.
- [x] Updated generated-code paths (`::bits::__private::*` → `::bnb::…`), all member
      `Cargo.toml`s, `[workspace.dependencies]`, the `deny.toml`/Cargo comments, and
      the root + crate `AGENTS.md`. (No protocol crate depended on `bits`, so the
      blast radius was just the two crates + workspace manifest. `refcheck` had no
      `bits` reference.)
- [x] Full gate green on default / `--features binrw` / `--features bytes` (44 test
      groups each), clippy deny, `cargo deny`, fmt. Memory + docs updated.

**Exit ✓ — the entire roadmap is complete:** an owned, bit-aware binary codec
(`bnb`, `#[bin]`) that subsumes our binrw usage; binrw is opt-in interop only.
