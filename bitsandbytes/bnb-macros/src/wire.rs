...
fn expand_inner(attr: TokenStream2, item: TokenStream2) -> syn::Result<TokenStream2> {
    ...
    let group_defs = groups.iter().map(|g| {
        ...
        let width_check = quote_spanned! {span=>
            ...
            const _: () = ::core::assert!(
                (0u32 #( + <#mtys as ::bnb::__private::Bits>::BITS )*) == <#backing>::BITS,
        ...
        quote! {
            #[::bnb::bitfield(#backing, bits = msb, bytes = #be_le)]
    ...
    let (validate_impl, post_build) = if let Some(path) = &args.validate {
        let v = quote! {
            impl #name {
                ...
                #vis fn validate(&self) -> ::core::result::Result<(), ::bnb::BuilderError> {
                    if self.check_soundness {
                        (#path)(self).map_err(|__e| ::bnb::BuilderError::invalid(__e.to_string()))?;
                    }
