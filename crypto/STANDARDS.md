...
| Local record contract | Fragmentation must not change records; completed records can be delivered one at a time to fallible caller output; a complete stream ends with one authenticated final record, including an empty final record at an exact boundary. | Staged `RecordBuilder`; `RecordSealer::{write_to,finish_to}` and collecting `write`/`finish`; `RecordSink`; distinct `DataRecord`/`FinalRecord`; public one-shot-versus-fragmented, sink-error and caught-panic invalidation, exact-boundary, order, and round-trip tests plus `aead_open` sink fuzz coverage. | Implemented and locally tested; not attributed to RFC 5116. |
