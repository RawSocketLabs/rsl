...
| Generic AEAD record completion | `RecordSealer::write_to` accepts arbitrary fragments and moves each completed record into a fallible `RecordSink`; a sink failure invalidates the stream. `finish_to` consumes the sealer and sends a separately typed authenticated final record. A decoder must require exactly one valid final record. Fixed fields must remain unique across streams under one key. | [AEAD records](src/aead/record.rs); [ledger](STANDARDS.md#generic-aead-record-source-baseline) |
