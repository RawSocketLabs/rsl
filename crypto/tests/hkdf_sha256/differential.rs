//! Differential comparison with the independent `RustCrypto` HKDF-SHA-256 implementation.

use hkdf::Hkdf;
use rsl_crypto::kdf::hkdf::sha256::derive;
use sha2::Sha256 as ReferenceSha256;
...
fn deterministic_inputs_match_rustcrypto() {
    for ikm_len in [0, 1, 22, 32, 80, 131] {
        ...
        for salt_len in [None, Some(0), Some(13), Some(80)] {
            ...
            for output_len in [0, 1, 31, 32, 33, 42, 82, 255] {
...
fn maximum_output_matches_rustcrypto() {
    compare_case(Some(b"salt"), b"input keying material", 255 * 32);
}

fn compare_case(salt: Option<&[u8]>, ikm: &[u8], output_len: usize) {
    let info = deterministic_bytes(67, 0xa9);
    let reference = Hkdf::<ReferenceSha256>::new(salt, ikm);
