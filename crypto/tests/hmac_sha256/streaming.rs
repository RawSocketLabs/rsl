...
fn common_key_and_message_representations_agree() {
    ...
    assert_eq!(
        HmacSha256::authenticate(key.as_bytes(), &message).map(HmacSha256Tag::into_bytes),
        Ok(expected)
    );
    assert_eq!(
        HmacSha256::authenticate(key.as_bytes(), message).map(HmacSha256Tag::into_bytes),
    ...
    assert_eq!(
        HmacSha256::authenticate(&key_bytes, message_bytes).map(HmacSha256Tag::into_bytes),
        Ok(expected)
    );
}
...
fn awkward_fragment_sizes_match_one_shot_authentication() {
    ...
    let expected =
        HmacSha256::authenticate(&key, &message).expect("the deterministic fixture is short");
...
fn empty_update_is_a_no_op() {
    ...
    assert_eq!(
        state.finalize(),
        HmacSha256::authenticate(b"key", "message").expect("the fixture is short")
    );
}

/// Authenticate one message in fixed-size fragments.
fn fragmented_tag(key: &[u8], message: &[u8], fragment_len: usize) -> HmacSha256Tag {
    let mut state = HmacSha256::new(key).expect("the deterministic key is short");
