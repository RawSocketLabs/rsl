//! Differential ECDH P-384 evidence against the `p256` crate 0.14.0.

use p384::{PublicKey, SecretKey, ecdh::diffie_hellman, elliptic_curve::sec1::ToSec1Point};
use rsl_crypto::agreement::ecdh_p384::{EcdhP384, EcdhP384PrivateKey, EcdhP384PublicKey};

fn deterministic_scalar(case: u8, multiplier: u8, offset: u8) -> [u8; 48] {
...
fn public_derivation_and_agreement_match_the_reference() {
    for case in 0_u8..32 {
        ...
        let ours = EcdhP384PrivateKey::from_bytes(private_bytes).unwrap();
        assert_eq!(
            EcdhP384::public_key(&ours).as_bytes().as_slice(),
            expected_public.as_bytes(),
            "public case {case}"
        );
        let peer = EcdhP384PublicKey::try_from(peer_public.as_bytes()).unwrap();
        assert_eq!(
            EcdhP384::agree(&ours, &peer)
                .unwrap()
                .expose_secret()
                .as_slice(),
...
fn derived_points_are_accepted_by_the_reference_parser() {
    for case in 0_u8..16 {
        let ours = EcdhP384PrivateKey::from_bytes(deterministic_scalar(case, 0x3d, 0x5a)).unwrap();
        let public = EcdhP384::public_key(&ours);
