//! Published RFC 8032 §7.4/§7.5 vectors, Wycheproof cases, and boundary evidence for Ed448.

use rsl_crypto::{
    CryptoError,
    digest::sha3::Shake256,
    signature::{
        Signer, Verifier,
        ed448::{Ed448Context, Ed448Signature, Ed448SigningKey, Ed448VerifyingKey},
    },
};

use crate::wycheproof_fixtures::CASES;

fn decode_vec(hex: &str) -> Vec<u8> {
    (0..hex.len() / 2)
        .map(|i| u8::from_str_radix(&hex[i * 2..i * 2 + 2], 16).expect("fixture is hex"))
        .collect()
}

fn decode<const N: usize>(hex: &str) -> [u8; N] {
    decode_vec(hex)
        .try_into()
        .unwrap_or_else(|_| panic!("fixture has {N} bytes"))
}

struct NoEntropy;

impl rsl_crypto::RandomSource for NoEntropy {
    fn fill_bytes(&mut self, _: &mut [u8]) -> rsl_crypto::Result<()> {
        Err(CryptoError::EntropyUnavailable)
    }
}

struct RfcCase {
    name: &'static str,
    prehashed: bool,
    secret: &'static str,
    public: &'static str,
    message: &'static str,
    context: &'static str,
    signature: &'static str,
}

/// Published evidence: every RFC 8032 §7.4 Ed448 and §7.5 Ed448ph vector is reproduced exactly
/// (key derivation and signature) and verifies.
#[test]
#[allow(clippy::too_many_lines)] // The published fixtures are long.
fn rfc_8032_ed448_and_ed448ph_vectors() {
    let cases = [
        RfcCase {
            name: "Blank",
            prehashed: false,
            secret: "6c82a562cb808d10d632be89c8513ebf6c929f34ddfa8c9f63c9960ef6e348a3528c8a3fcc2f044e39a3fc5b94492f8f032e7549a20098f95b",
            public: "5fd7449b59b461fd2ce787ec616ad46a1da1342485a70e1f8a0ea75d80e96778edf124769b46c7061bd6783df1e50f6cd1fa1abeafe8256180",
            message: "",
            context: "",
            signature: "533a37f6bbe457251f023c0d88f976ae2dfb504a843e34d2074fd823d41a591f2b233f034f628281f2fd7a22ddd47d7828c59bd0a21bfd3980ff0d2028d4b18a9df63e006c5d1c2d345b925d8dc00b4104852db99ac5c7cdda8530a113a0f4dbb61149f05a7363268c71d95808ff2e652600",
        },
        RfcCase {
            name: "1 octet",
            prehashed: false,
            secret: "c4eab05d357007c632f3dbb48489924d552b08fe0c353a0d4a1f00acda2c463afbea67c5e8d2877c5e3bc397a659949ef8021e954e0a12274e",
            public: "43ba28f430cdff456ae531545f7ecd0ac834a55d9358c0372bfa0c6c6798c0866aea01eb00742802b8438ea4cb82169c235160627b4c3a9480",
            message: "03",
            context: "",
            signature: "26b8f91727bd62897af15e41eb43c377efb9c610d48f2335cb0bd0087810f4352541b143c4b981b7e18f62de8ccdf633fc1bf037ab7cd779805e0dbcc0aae1cbcee1afb2e027df36bc04dcecbf154336c19f0af7e0a6472905e799f1953d2a0ff3348ab21aa4adafd1d234441cf807c03a00",
        },
        RfcCase {
            name: "1 octet (with context)",
            prehashed: false,
            secret: "c4eab05d357007c632f3dbb48489924d552b08fe0c353a0d4a1f00acda2c463afbea67c5e8d2877c5e3bc397a659949ef8021e954e0a12274e",
            public: "43ba28f430cdff456ae531545f7ecd0ac834a55d9358c0372bfa0c6c6798c0866aea01eb00742802b8438ea4cb82169c235160627b4c3a9480",
            message: "03",
            context: "666f6f",
            signature: "d4f8f6131770dd46f40867d6fd5d5055de43541f8c5e35abbcd001b32a89f7d2151f7647f11d8ca2ae279fb842d607217fce6e042f6815ea000c85741de5c8da1144a6a1aba7f96de42505d7a7298524fda538fccbbb754f578c1cad10d54d0d5428407e85dcbc98a49155c13764e66c3c00",
        },
        RfcCase {
            name: "11 octets",
            prehashed: false,
            secret: "cd23d24f714274e744343237b93290f511f6425f98e64459ff203e8985083ffdf60500553abc0e05cd02184bdb89c4ccd67e187951267eb328",
            public: "dcea9e78f35a1bf3499a831b10b86c90aac01cd84b67a0109b55a36e9328b1e365fce161d71ce7131a543ea4cb5f7e9f1d8b00696447001400",
            message: "0c3e544074ec63b0265e0c",
            context: "",
            signature: "1f0a8888ce25e8d458a21130879b840a9089d999aaba039eaf3e3afa090a09d389dba82c4ff2ae8ac5cdfb7c55e94d5d961a29fe0109941e00b8dbdeea6d3b051068df7254c0cdc129cbe62db2dc957dbb47b51fd3f213fb8698f064774250a5028961c9bf8ffd973fe5d5c206492b140e00",
        },
        RfcCase {
            name: "12 octets",
            prehashed: false,
            secret: "258cdd4ada32ed9c9ff54e63756ae582fb8fab2ac721f2c8e676a72768513d939f63dddb55609133f29adf86ec9929dccb52c1c5fd2ff7e21b",
            public: "3ba16da0c6f2cc1f30187740756f5e798d6bc5fc015d7c63cc9510ee3fd44adc24d8e968b6e46e6f94d19b945361726bd75e149ef09817f580",
            message: "64a65f3cdedcdd66811e2915",
            context: "",
            signature: "7eeeab7c4e50fb799b418ee5e3197ff6bf15d43a14c34389b59dd1a7b1b85b4ae90438aca634bea45e3a2695f1270f07fdcdf7c62b8efeaf00b45c2c96ba457eb1a8bf075a3db28e5c24f6b923ed4ad747c3c9e03c7079efb87cb110d3a99861e72003cbae6d6b8b827e4e6c143064ff3c00",
        },
        RfcCase {
            name: "13 octets",
            prehashed: false,
            secret: "7ef4e84544236752fbb56b8f31a23a10e42814f5f55ca037cdcc11c64c9a3b2949c1bb60700314611732a6c2fea98eebc0266a11a93970100e",
            public: "b3da079b0aa493a5772029f0467baebee5a8112d9d3a22532361da294f7bb3815c5dc59e176b4d9f381ca0938e13c6c07b174be65dfa578e80",
            message: "64a65f3cdedcdd66811e2915e7",
            context: "",
            signature: "6a12066f55331b6c22acd5d5bfc5d71228fbda80ae8dec26bdd306743c5027cb4890810c162c027468675ecf645a83176c0d7323a2ccde2d80efe5a1268e8aca1d6fbc194d3f77c44986eb4ab4177919ad8bec33eb47bbb5fc6e28196fd1caf56b4e7e0ba5519234d047155ac727a1053100",
        },
        RfcCase {
            name: "64 octets",
            prehashed: false,
            secret: "d65df341ad13e008567688baedda8e9dcdc17dc024974ea5b4227b6530e339bff21f99e68ca6968f3cca6dfe0fb9f4fab4fa135d5542ea3f01",
            public: "df9705f58edbab802c7f8363cfe5560ab1c6132c20a9f1dd163483a26f8ac53a39d6808bf4a1dfbd261b099bb03b3fb50906cb28bd8a081f00",
            message: "bd0f6a3747cd561bdddf4640a332461a4a30a12a434cd0bf40d766d9c6d458e5512204a30c17d1f50b5079631f64eb3112182da3005835461113718d1a5ef944",
            context: "",
            signature: "554bc2480860b49eab8532d2a533b7d578ef473eeb58c98bb2d0e1ce488a98b18dfde9b9b90775e67f47d4a1c3482058efc9f40d2ca033a0801b63d45b3b722ef552bad3b4ccb667da350192b61c508cf7b6b5adadc2c8d9a446ef003fb05cba5f30e88e36ec2703b349ca229c2670833900",
        },
        RfcCase {
            name: "256 octets",
            prehashed: false,
            secret: "2ec5fe3c17045abdb136a5e6a913e32ab75ae68b53d2fc149b77e504132d37569b7e766ba74a19bd6162343a21c8590aa9cebca9014c636df5",
            public: "79756f014dcfe2079f5dd9e718be4171e2ef2486a08f25186f6bff43a9936b9bfe12402b08ae65798a3d81e22e9ec80e7690862ef3d4ed3a00",
            message: "",
            context: "",
            signature: "c650ddbb0601c19ca11439e1640dd931f43c518ea5bea70d3dcde5f4191fe53f00cf966546b72bcc7d58be2b9badef28743954e3a44a23f880e8d4f1cfce2d7a61452d26da05896f0a50da66a239a8a188b6d825b3305ad77b73fbac0836ecc60987fd08527c1a8e80d5823e65cafe2a3d00",
        },
        RfcCase {
            name: "1023 octets",
            prehashed: false,
            secret: "872d093780f5d3730df7c212664b37b8a0f24f56810daa8382cd4fa3f77634ec44dc54f1c2ed9bea86fafb7632d8be199ea165f5ad55dd9ce8",
            public: "a81b2e8a70a5ac94ffdbcc9badfc3feb0801f258578bb114ad44ece1ec0e799da08effb81c5d685c0c56f64eecaef8cdf11cc38737838cf400",
            message: "",
            context: "",
            signature: "e301345a41a39a4d72fff8df69c98075a0cc082b802fc9b2b6bc503f926b65bddf7f4c8f1cb49f6396afc8a70abe6d8aef0db478d4c6b2970076c6a0484fe76d76b3a97625d79f1ce240e7c576750d295528286f719b413de9ada3e8eb78ed573603ce30d8bb761785dc30dbc320869e1a00",
        },
        RfcCase {
            name: "TEST abc",
            prehashed: true,
            secret: "833fe62409237b9d62ec77587520911e9a759cec1d19755b7da901b96dca3d42ef7822e0d5104127dc05d6dbefde69e3ab2cec7c867c6e2c49",
            public: "259b71c19f83ef77a7abd26524cbdb3161b590a48f7d17de3ee0ba9c52beb743c09428a131d6b1b57303d90d8132c276d5ed3d5d01c0f53880",
            message: "616263",
            context: "",
            signature: "822f6901f7480f3d5f562c592994d9693602875614483256505600bbc281ae381f54d6bce2ea911574932f52a4e6cadd78769375ec3ffd1b801a0d9b3f4030cd433964b6457ea39476511214f97469b57dd32dbc560a9a94d00bff07620464a3ad203df7dc7ce360c3cd3696d9d9fab90f00",
        },
        RfcCase {
            name: "TEST abc (with context)",
            prehashed: true,
            secret: "833fe62409237b9d62ec77587520911e9a759cec1d19755b7da901b96dca3d42ef7822e0d5104127dc05d6dbefde69e3ab2cec7c867c6e2c49",
            public: "259b71c19f83ef77a7abd26524cbdb3161b590a48f7d17de3ee0ba9c52beb743c09428a131d6b1b57303d90d8132c276d5ed3d5d01c0f53880",
            message: "616263",
            context: "666f6f",
            signature: "c32299d46ec8ff02b54540982814dce9a05812f81962b649d528095916a2aa481065b1580423ef927ecf0af5888f90da0f6a9a85ad5dc3f280d91224ba9911a3653d00e484e2ce232521481c8658df304bb7745a73514cdb9bf3e15784ab71284f8d0704a608c54a6b62d97beb511d132100",
        },
    ];
    for case in &cases {
        let signing = Ed448SigningKey::from_seed(decode(case.secret));
        let public: [u8; 57] = decode(case.public);
        assert_eq!(
            signing.verifying_key().into_bytes(),
            public,
            "{} public key",
            case.name
        );
        let message = decode_vec(case.message);
        let context = if case.context.is_empty() {
            None
        } else {
            Some(Ed448Context::new(&decode_vec(case.context)).unwrap())
        };
        let expected = Ed448Signature::from_bytes(decode(case.signature));
        let verifying = Ed448VerifyingKey::from_bytes(public).unwrap();
        if case.prehashed {
            let mut prehash = [0_u8; 64];
            Shake256::digest_into(&message, &mut prehash);
            let signature = signing.sign_prehashed(&prehash, context.as_ref()).unwrap();
            assert_eq!(signature, expected, "{} signature", case.name);
            verifying
                .verify_prehashed(&prehash, context.as_ref(), &expected)
                .unwrap_or_else(|_| panic!("{} verifies", case.name));
        } else {
            let signature = signing.sign(context.as_ref(), &message).unwrap();
            assert_eq!(signature, expected, "{} signature", case.name);
            verifying
                .verify(context.as_ref(), &message, &expected)
                .unwrap_or_else(|_| panic!("{} verifies", case.name));
        }
    }
}

/// Published evidence: every Wycheproof `ed448` result is reproduced.
#[test]
fn wycheproof_results_are_reproduced() {
    let mut valid = 0;
    for case in &CASES {
        let key = Ed448VerifyingKey::from_bytes(decode(case.public_key))
            .unwrap_or_else(|_| panic!("tcId {} public key", case.tc_id));
        let message = decode_vec(case.msg);
        let signature = decode_vec(case.sig);
        let outcome = Ed448Signature::try_from(signature.as_slice())
            .and_then(|signature| key.verify(None, &message, &signature));
        let expected_valid = case.result == "valid";
        assert_eq!(
            outcome.is_ok(),
            expected_valid,
            "tcId {} ({}; {})",
            case.tc_id,
            case.comment,
            case.flags
        );
        if expected_valid {
            valid += 1;
        }
    }
    assert_eq!(valid, 17, "Wycheproof publishes 17 valid Ed448 cases");
}

/// Standard-derived evidence: contexts, wire lengths, non-canonical `S`, tampering, and the
/// generic contracts.
#[test]
fn boundaries_and_generic_contracts() {
    assert!(Ed448Context::new(b"").is_err());
    assert!(Ed448Context::new(&[0; 256]).is_err());
    assert_eq!(
        Ed448VerifyingKey::try_from([0_u8; 32].as_slice()),
        Err(CryptoError::InvalidLength {
            name: "Ed448 public key",
            expected: 57,
            actual: 32,
        })
    );
    assert_eq!(
        Ed448Signature::try_from([0_u8; 64].as_slice()),
        Err(CryptoError::InvalidLength {
            name: "Ed448 signature",
            expected: 114,
            actual: 64,
        })
    );

    let signing = Ed448SigningKey::from_seed([0x24; 57]);
    let verifying = signing.verifying_key();
    let message = b"same message";
    let signature = signing.sign(None, message).unwrap();
    assert_eq!(
        Signer::sign(&signing, &mut NoEntropy, message).unwrap(),
        signature
    );
    Verifier::verify(&verifying, message, &signature).unwrap();
    assert!(Ed448SigningKey::generate(&mut NoEntropy).is_err());

    for byte_index in [0, 56, 57, 113] {
        let mut changed = signature.into_bytes();
        changed[byte_index] ^= 0x01;
        assert!(
            verifying
                .verify(None, message, &Ed448Signature::from_bytes(changed))
                .is_err(),
            "byte {byte_index}"
        );
    }
    let mut noncanonical_s = signature.into_bytes();
    noncanonical_s[113] |= 0x80;
    assert_eq!(
        verifying.verify(None, message, &Ed448Signature::from_bytes(noncanonical_s)),
        Err(CryptoError::InvalidSignature)
    );
    let foo = Ed448Context::new(b"foo").unwrap();
    assert!(verifying.verify(Some(&foo), message, &signature).is_err());
    let mut prehash = [0_u8; 64];
    Shake256::digest_into(message, &mut prehash);
    assert!(
        verifying
            .verify_prehashed(&prehash, None, &signature)
            .is_err()
    );
}
