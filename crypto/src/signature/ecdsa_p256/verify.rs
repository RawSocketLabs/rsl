//! FIPS 186-5 §6.4.2 ECDSA signature verification steps for P-256.
//!
...
pub(super) fn verify_digest(
    ...
    // Step 2: `e` is the leftmost `min(N, outlen)` bits of `H(M)`; with SHA-256 that is all 256
    ...
    let r_point = ProjectivePoint::generator()
        .multiply(&u1.to_bytes())
        .add(&q.to_projective().multiply(&u2.to_bytes()));
