//! ECDSA over P-256 with SHA-256, taught from a digest to a checked point.
//!
//! # What ECDSA P-256 does
//!
...
//! SP 800-186 supplies the curve. TLS 1.3 names this profile `ecdsa_secp256r1_sha256`; SSH
//! names it `ecdsa-sha2-nistp256`.
//!
...
//!
//! - [`EcdsaP256SigningKey`] owns one scalar in `[1, n-1]`; construction rejects zero and
...
//! | --- | --- | --- |
//! | `d`, `x` | [`EcdsaP256SigningKey`] | The private scalar. |
//! | `Q` | [`EcdsaP256VerifyingKey`] | The signer's public point. |
//! | `(r, s)` | [`EcdsaP256Signature`] | The signature, two scalars in `[1, n-1]`. |
//! | `H(M)`, `e`, `h1` | [`Sha256Digest`](crate::digest::sha2::sha256::Sha256Digest) then `Scalar::reduce_bytes` | The 256-bit digest as an integer modulo `n`. |
//! | `K`, `V`, `T` | private [`nonce::NonceGenerator`] | RFC 6979 HMAC-SHA-256 state and candidate. |
...
//! # Signing walkthrough
//!
//! 1. Hash the message with SHA-256 to `h1`; because the digest and `n` are both 256 bits,
//!    RFC 6979's `bits2int` keeps the whole digest and `bits2octets(h1)` is `h1 mod n`.
//! 2. Seed `V = 0x01…`, `K = 0x00…`, then apply the RFC's steps d–g with HMAC-SHA-256 over
...
//! 2. Hash the message with SHA-256 and read the digest as a big-endian integer `e`.
...
//!
//! RFC 6979 A.2.5 publishes a private key and the deterministic SHA-256 signature of
...
//! }
//!
//! let key = EcdsaP256SigningKey::from_bytes(decode(
//!     "C9AFA9D845BA75166B5C215767B1D6934E50C3DB36E89B127B8A622B120F6721",
//! ))?;
//! let signature = key.sign_sha256(b"sample")?;
//!
//! let mut expected = [0_u8; 64];
//! expected[..32].copy_from_slice(&decode::<32>(
...
//!
//! key.verifying_key().verify_sha256(b"sample", &signature)?;
...
//!
//! Signing evidence: RFC 6979 A.2.5's published `k` values and both SHA-256 signatures are
//! reproduced exactly, and all 15 NIST CAVP `SigGen` P-256/SHA-256 cases reproduce `(r, s)`
...
pub use api::{EcdsaP256Signature, EcdsaP256SigningKey, EcdsaP256VerifyingKey};
