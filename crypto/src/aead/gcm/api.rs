//! Public AES-128-GCM authenticated-encryption boundary.
//!
//! ## Profile and standards mapping
//!
//! [`Aes128Gcm`] composes AES-128 with the GCM authenticated-encryption and authenticated-
//! decryption functions in [NIST SP 800-38D §7.1–7.2][sp-800-38d]. The first public profile is
//! intentionally exact and narrow:
//!
//! - [`Aes128GcmKey`] owns exactly the 128 key bits required by AES-128;
//! - [`Aes128GcmNonce`] owns exactly the 96 IV bits recommended by §5.2.1.1; and
//! - [`Aes128GcmTag`] contains the complete 128-bit tag, with no caller-selected truncation.
//!
//! The nonce is called an IV in SP 800-38D. This API uses the modern AEAD term `nonce` to emphasize
//! the §8 requirement that a key/nonce pair must not be repeated. The type fixes its size but
//! cannot prove uniqueness; TLS, SSH, or another consuming protocol must construct nonces from its
//! own sequence state and enforce key lifetime and exhaustion rules.
//!
//! Additional authenticated data is authenticated but remains unencrypted. This is the intended
//! boundary for cleartext record or packet fields: a protocol can send those encoded bytes
//! directly while binding the exact same bytes as AAD. Choosing which bytes are cleartext and
//! constructing their wire representation remain protocol responsibilities.
//!
//! ## Plaintext release
//!
//! [`Aes128Gcm::open`] authenticates the complete IV, AAD, and ciphertext before allocating or
//! transforming its plaintext output. Failure returns only
//! [`crate::CryptoError::AuthenticationFailed`] and releases no plaintext.
//!
//! ## Security status
//!
//! This is an accuracy-first reference implementation. Its source is structured for review, but
//! it has not undergone independent audit, compiler-output side-channel analysis, or formal NIST
//! validation. See the crate `README.md` and `STANDARDS.md` for the evidence ledger and remaining
//! work.
//!
...
use crate::{
    ...
    aead::{Aead, Sealed},
    cipher::aes::aes128::{Aes128, Aes128Key},
    random::RandomSource,
};

/// Number of bytes in an AES-128-GCM key.
...
/// An owned AES-128 key dedicated to GCM.
///
/// This type intentionally implements neither `Clone`, `Copy`, `AsRef`, nor `Deref`. Constructing
/// [`Aes128Gcm`] consumes it, expands it once, and clears the input owner. Keeping it distinct from
/// [`Aes128Key`] reflects SP 800-38D §5.1's requirement that a GCM key be used exclusively for
/// GCM with its chosen block cipher.
///
/// # Examples
///
/// ```
/// use rsl_crypto::aead::gcm::{Aes128Gcm, Aes128GcmKey};
///
/// let key = Aes128GcmKey::new([0x42; 16]);
/// let algorithm = Aes128Gcm::new(key); // consumes and expands the dedicated key
/// assert_eq!(format!("{algorithm:?}"), "Aes128Gcm([REDACTED KEY SCHEDULE])");
/// ```
...
impl Aes128GcmKey {
    /// Size of an AES-128-GCM key in bytes.
    pub const LEN: usize = KEY_BYTES;

    /// Take ownership of exactly 128 key bits.
    #[must_use]
    ...
    /// Generate a dedicated AES-128-GCM key with the caller-selected randomness source.
    ///
...
impl fmt::Debug for Aes128GcmKey {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("Aes128GcmKey([REDACTED])")
    }
}

/// One owned 96-bit AES-128-GCM nonce.
///
...
///
/// ```
/// use rsl_crypto::aead::gcm::Aes128GcmNonce;
///
/// let wire_bytes = [0x24; 12];
/// let nonce = Aes128GcmNonce::try_from(wire_bytes.as_slice())?;
/// assert_eq!(nonce.as_bytes(), &wire_bytes);
/// # Ok::<(), rsl_crypto::CryptoError>(())
/// ```
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Aes128GcmNonce([u8; NONCE_BYTES]);
...
impl AsRef<[u8]> for Aes128GcmNonce {
    fn as_ref(&self) -> &[u8] {
        &self.0
    }
}

impl TryFrom<&[u8]> for Aes128GcmNonce {
...
/// One complete 128-bit AES-128-GCM authentication tag.
///
...
///
/// ```
/// use rsl_crypto::aead::gcm::Aes128GcmTag;
///
/// let wire_bytes = [0xa5; 16];
/// let tag = Aes128GcmTag::try_from(wire_bytes.as_slice())?;
/// assert_eq!(tag.into_bytes(), wire_bytes);
/// # Ok::<(), rsl_crypto::CryptoError>(())
/// ```
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Aes128GcmTag([u8; TAG_BYTES]);
...
impl AsRef<[u8]> for Aes128GcmTag {
    fn as_ref(&self) -> &[u8] {
        &self.0
    }
}

impl TryFrom<&[u8]> for Aes128GcmTag {
    type Error = CryptoError;
...
/// AES-128-GCM with one expanded, zeroizing key schedule.
///
...
impl Aes128Gcm {
    /// Consume and expand one dedicated AES-128-GCM key.
    #[must_use]
    pub fn new(key: Aes128GcmKey) -> Self {
        let Aes128GcmKey { bytes } = key;
        let cipher = Aes128::new(Aes128Key::new(bytes.into_inner()));
    ...
    pub fn seal(
        ...
        Ok(Sealed::new(ciphertext, Aes128GcmTag::new(tag)))
    }
...
impl fmt::Debug for Aes128Gcm {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("Aes128Gcm([REDACTED KEY SCHEDULE])")
    }
}
...
mod unit {
    ...
    use super::{Aes128Gcm, Aes128GcmKey, Aes128GcmNonce, Aes128GcmTag};
    use crate::CryptoError;

    #[test]
    fn public_sizes_are_exact_and_byte_exposure_is_explicit() {
        let nonce_bytes = [0x12; Aes128GcmNonce::LEN];
        let tag_bytes = [0x34; Aes128GcmTag::LEN];
        let nonce = Aes128GcmNonce::new(nonce_bytes);
        let tag = Aes128GcmTag::new(tag_bytes);

        assert_eq!(Aes128GcmKey::LEN, 16);
    ...
    fn key_owning_debug_output_is_redacted() {
        let key = Aes128GcmKey::new([0x5a; 16]);
        assert_eq!(format!("{key:?}"), "Aes128GcmKey([REDACTED])");

        let algorithm = Aes128Gcm::new(key);
        assert_eq!(
            format!("{algorithm:?}"),
            "Aes128Gcm([REDACTED KEY SCHEDULE])"
        );
    }
    ...
    fn wire_slices_require_exact_nonce_and_tag_lengths() {
        ...
        assert_eq!(
            Aes128GcmNonce::try_from(nonce_bytes.as_slice()),
            Ok(Aes128GcmNonce::new(nonce_bytes))
        );
        assert_eq!(
            Aes128GcmTag::try_from(tag_bytes.as_slice()),
            Ok(Aes128GcmTag::new(tag_bytes))
        );

        assert_eq!(
            Aes128GcmNonce::try_from(&nonce_bytes[..11]),
